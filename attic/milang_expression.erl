...
parse(Input) when is_binary(Input) ->
  _ = setup_memo(),
  Result = case 'expression_fake_head'(Input,{{line,1},{column,1}}) of
             {AST, <<>>, _Index} -> AST;
             Any -> Any
           end,
  release_memo(), Result.

-spec 'expression_fake_head'(input(), index()) -> parse_result().
'expression_fake_head'(Input, Index) ->
  p(Input, Index, 'expression_fake_head', fun(I,D) -> (p_choose([fun 'expression'/2, fun 'variable'/2, fun 'bindable_function_name'/2]))(I,D) end, fun(Node, _Idx) ->Node end).
